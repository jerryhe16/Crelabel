...
- 当前仓库默认仅在本机使用。配置远程仓库和推送代码必须由用户明确指定平台和目标后再执行。
- 用户日后若把本仓库放到 GitHub，其他代理仍以本文件和 `HANDOFF.md` 为唯一入口：先读这两份文件，再从最新 `main` 开自己的 `codex/<task>` 或 `grok/<task>` 分支。不要假设聊天记录仍可用。
- 出现 GitHub 远程之后仍然禁止：force-push `main`、改写已共享历史、在未登记时改同一批文件、提交 `release/` `dist/` `build/` 或授权信息。推送哪一个远程、是否开 Pull Request，必须由用户当次指定。
